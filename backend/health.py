...
class BackendHealthMonitor:
    """Poll backend runtime health and publish summarized health state.

    Scope for this commit:
    - writer process / queue / drop health from HistoryManager
    - bus health from StateStore
    - script health from StateStore
    - change-triggered system_event emission
    """
    ...
    def _run(self) -> None:
        while not self._stop_event.wait(self.poll_interval_s):
            try:
                ...
                # Health monitoring must never take backend down.
                pass
    ...
    def _normalize_writer_health(self, side_name: str, entry: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if utilization is not None and utilization >= self.queue_warning_fraction:
            queue_warning = (
                f"{side_name} queue at {utilization * 100:.0f}% "
    ...
    def _emit_transition_events(self, snapshot: Mapping[str, Any]) -> None:
        ...
        if isinstance(writers, Mapping):
            for side_name, entry in writers.items():
                ...
                if previous != signature:
                    severity = "error" if entry.get("status") == "error" else (
                        "warning" if entry.get("status") == "warning" else "info"
                    )
        ...
        if isinstance(bus, Mapping):
            ...
            if self._last_bus_signature != bus_signature:
                severity = "error" if bus.get("status") == "error" else (
                    "warning" if bus.get("status") == "warning" else "info"
                )
                self.health_publisher.record_system_event(
                    "bus_health_changed",
                    severity=severity,
        ...
        if self._last_overall_signature != overall_signature:
            severity = "error" if snapshot.get("overall_status") == "error" else (
