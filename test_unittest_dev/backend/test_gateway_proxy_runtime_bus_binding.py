...
class TestGatewayProxyRuntimeBusBinding(unittest.TestCase):
    def _make_service_like(self):
        ...
        def _fake_attach(registered_ids):
            for device_id in registered_ids:
                runtime = svc.device_registry.get_runtime(device_id)
                runtime._bus = _SpyGatewayBus()

        svc._attach_gateway_bus_proxies = _fake_attach
        svc._detach_all_gateway_bus_proxies = lambda: None

        return svc

    def test_gateway_status_binds_runtime_bus_for_registered_solenoid(self):
        svc = self._make_service_like()

        runtime = svc.device_registry.get_runtime("ig-xv-24")
        self.assertIsNone(getattr(runtime, "_bus", None))
        ...
        runtime = svc.device_registry.get_runtime("ig-xv-24")
        self.assertIsNotNone(runtime._bus)
        self.assertTrue(hasattr(runtime._bus, "send"))

        runtime.open()
        self.assertEqual(len(runtime._bus.calls), 1)