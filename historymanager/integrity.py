...
class ResolvedRunPaths:
    """Resolved archive roots for a single run.

    Attributes:
        project_root: Absolute project root that owns the archive directories.
        run_id: Canonical run identifier derived from the reference.
        raw_dir: Raw archive directory for the run under ``.ignitionraw``.
        rawbak_dir: Backup raw archive directory for the run under
            ``.ignitionrawbak``.
        history_dir: Structured history directory for the run under
            ``ignitionhistory``.
    """
...
def utc_now_iso() -> str:
    """Return the current UTC timestamp in archive-friendly ISO-8601 form.

    Returns:
        The current UTC time formatted with millisecond precision and a trailing
        ``Z`` suffix.
    """
    return (
...
def scan_run_integrity(
    ...
    *,
    ...
    """Scan a run archive and build its integrity report payload.

    The scan compares shared streams across raw, raw-backup, and structured
    history artifacts, then falls back to source-specific checks for raw-only
    and structured-only streams. The resulting payload is the in-memory form of
    ``integrity_report.json``.

    Args:
        run_ref: Run identifier or path pointing at a run directory or file
            within a run directory.
        project_root: Project root that owns the archive directories.

    Returns:
        A report payload containing source presence, per-stream integrity
        summaries, and an overall badge and summary message for the run.
    """
    ...
    source_presence = {
        source_name: root.is_dir() for source_name, root in source_roots.items()
    }
...
def write_run_integrity_report(
    ...
    *,
    ...
    """Write ``integrity_report.json`` for a run's structured history directory.

    Args:
        run_ref: Run identifier or path pointing at a run directory or file
            within a run directory.
        project_root: Project root that owns the archive directories.
        report: Precomputed report payload to write. When omitted, the report is
            generated by scanning the run inline.

    Returns:
        The path to the written ``integrity_report.json`` file.

    Raises:
        FileNotFoundError: If the run's structured history directory does not
            exist.
    """
    paths = _resolve_run_paths(run_ref, project_root=project_root)
    payload = (
        ...
        else scan_run_integrity(run_ref, project_root=project_root)
    )
...
def scan_and_write_run_integrity(
    ...
    *,
    ...
    """Scan a run archive and persist the resulting integrity report.
    ...
    Returns:
        A tuple of the generated report payload and the written report path.
    """
...
def _resolve_run_paths(
    ...
    """Resolve the raw, raw-backup, and structured directories for a run.
    ...
    Returns:
        A resolved path bundle for the referenced run.
    """
...
def _scan_stream_file(
    *, source_name: str, stream_name: str, path: Path
) -> dict[str, Any]:
    """Scan one archive stream file for event identity and sequence issues.

    The scan tracks parse errors, missing canonical identity fields, duplicate
    event UIDs, first and last ``recorded_at`` values, and a per-event index
    keyed by ``event_uid`` for later cross-source comparison.

    Args:
        source_name: Archive source label such as ``raw``, ``rawbak``, or
            ``history``.
        stream_name: Canonical stream name being scanned.
        path: Path to the JSONL file for that stream.

    Returns:
        A detailed scan dictionary used as input to stream-level integrity
        reporting.
    """
    result: dict[str, Any] = {
    ...
    result["sequence_issue_count"], result["sequence_issue_sample"] = (
        _scan_sequence_issues(stream_seqs)
    )
    return result


def _scan_sequence_issues(stream_seqs: list[int]) -> tuple[int, list[dict[str, Any]]]:
    """Summarize gaps or invalid starts in a stream sequence series.

    Args:
        stream_seqs: Observed ``stream_seq`` values for identity-bearing events.

    Returns:
        A tuple of the total issue count and a capped sample describing the
        first detected sequence problems.
    """
    if not stream_seqs:
        return 0, []
...
def _build_stream_report(
    *,
    ...
    """Build the integrity summary for one logical stream across its sources.

    Args:
        stream_name: Canonical stream name being summarized.
        source_scans: Per-source scan results keyed by source name.

    Returns:
        A stream report with source summaries, missing-event samples,
        cross-source hash and sequence mismatch samples, and a status badge.
    """
    ...
    has_parse_errors = any(
        scan["parse_error_count"] > 0 for scan in source_scans.values()
    )
    has_missing_identity = any(
        scan["missing_identity_count"] > 0 for scan in source_scans.values()
    )
    has_duplicates = any(
        scan["duplicate_event_uid_count"] > 0 for scan in source_scans.values()
    )
    has_sequence_issues = any(
        scan["sequence_issue_count"] > 0 for scan in source_scans.values()
    )
    ...
    if present_source_count == 0:
        ...
        has_parse_errors
        or has_missing_identity
        or has_duplicates
        or has_hash_mismatch
        or has_seq_mismatch
    ):
        status = "mismatch"
        badge = "red"
        ...
        badge = "yellow"
        missing_sources = [
            name for name, scan in source_scans.items() if not scan["present"]
        ]
        if missing_sources:
            joined = ", ".join(missing_sources)
            message = (
                f"Missing from {joined}, but rest data matches for stream {stream_name}"
            )
        else:
...
def _summarize_source_scan(scan: dict[str, Any]) -> dict[str, Any]:
    """Drop the per-event index from a source scan and keep report-facing fields.

    Args:
        scan: Detailed scan result produced by ``_scan_stream_file``.

    Returns:
        A reduced scan summary suitable for embedding in the final report.
    """
    return {
        "path": scan["path"],
...
def _build_overall_report(
    *,
    ...
    """Collapse per-stream findings into the run-level integrity status.

    Args:
        run_id: Run identifier used in the fallback unknown message.
        source_presence: Whether each archive root exists for the run.
        stream_reports: Per-stream integrity reports.

    Returns:
        The overall status, badge, and summary message for the run.
    """
...
def _atomic_write_json(path: Path, payload: dict[str, Any]) -> None:
    """Write a JSON file atomically through a temporary sibling path.
